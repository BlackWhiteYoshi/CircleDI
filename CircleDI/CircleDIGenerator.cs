...
public sealed class CircleDIGenerator : IIncrementalGenerator {
    ...
    private void GenerateClass(SourceProductionContext context, ServiceProvider serviceProvider) {
        ...
        // class head
        builderExtension.AppendClassSummary(serviceProvider.GenerateDisposeMethods, serviceProvider.ThreadSafe);
        ...
        if (!serviceProvider.HasConstructor) {
            ...
            builder.Append($$"""
                ...
                """);
            builderExtension.AppendInitServicesMemberNotNull(serviceProvider.SingletonList, isScopeProvider: false);
        ...
        builderExtension.AppendServicesGetter(serviceProvider.SingletonList, serviceProvider.ThreadSafe);

        // Transient services
        (bool hasDisposeList, bool hasAsyncDisposeList) = builderExtension.AppendServicesTransient(serviceProvider.GenerateDisposeMethods, isScopeProvider: false);

        // Delegate services
        builderExtension.AppendServicesDelegate(isScopeProvider: false);

        // IServiceProvider
        builderExtension.AppendIServiceProviderNotScoped();

        // Dispose
        builderExtension.AppendDisposeMethods(serviceProvider.SingletonList, serviceProvider.GenerateDisposeMethods, serviceProvider.ThreadSafe, hasDisposeList, hasAsyncDisposeList, serviceProvider.HasDisposeMethod, serviceProvider.HasDisposeAsyncMethod);

        // UnsafeAccessor methods
        builderExtension.AppendUnsafeAccessorMethods(serviceProvider.SingletonList);


        // Scope
        if (serviceProvider.GenerateScope) {
            builderExtension.indent.InitPlusOneIndent();
            builder.Append('\n');

            // class head
            builderExtension.AppendClassSummary(serviceProvider.GenerateDisposeMethodsScope, serviceProvider.ThreadSafeScope);
            ...
            if (!serviceProvider.HasConstructorScope) {
                ...
                builder.Append($$"""
                    ...
                    """);
                builderExtension.AppendInitServicesMemberNotNull(serviceProvider.ScopedList, isScopeProvider: true);
            ...
            builderExtension.AppendConstructorServices(serviceProvider.ScopedList);
            ...
            // Transient services
            (bool hasDisposeListScope, bool hasAsyncDisposeListScope) = builderExtension.AppendServicesTransient(serviceProvider.GenerateDisposeMethodsScope, isScopeProvider: true);

            // Delegate services
            builderExtension.AppendServicesDelegate(isScopeProvider: true);

            // IServiceProvider
            builderExtension.AppendIServiceProviderAllServices();

            // Dispose
            builderExtension.AppendDisposeMethods(serviceProvider.ScopedList, serviceProvider.GenerateDisposeMethodsScope, serviceProvider.ThreadSafeScope, hasDisposeListScope, hasAsyncDisposeListScope, serviceProvider.HasDisposeMethodScope, serviceProvider.HasDisposeAsyncMethodScope);

            // UnsafeAccessor methods
            builderExtension.AppendUnsafeAccessorMethods(serviceProvider.ScopedList);
    ...
    private struct StringBuilderExtension(StringBuilder builder, ServiceProvider serviceProvider) {
        ...
        public readonly void AppendInitServicesMemberNotNull(List<Service> serviceList, bool isScopeProvider) {
        ...
        /// </summary>
        /// <param name="serviceList"><see cref="ServiceProvider.SingletonList"/> or <see cref="ServiceProvider.ScopedList"/><</param>
        public readonly void AppendConstructorServices(List<Service> serviceList) {
        ...
        public void AppendServicesGetter(List<Service> serviceList, bool threadSafe) {
            if (serviceList.Count > 0) {
                foreach (Service service in serviceList) {
                    ...
                    if (service.Implementation.Type == MemberType.Field)
                        AppendServicesGetterField(service);
                    else if (service.CreationTime == CreationTiming.Constructor)
                        AppendServicesGetterConstructor(service);
                    else
                        AppendServicesGetterLazy(service, threadSafe);
        ...
        private readonly void AppendServicesGetterField(Service service) {
            builder.Append(" => ");
            if (service.Lifetime == ServiceLifetime.Scoped && !service.Implementation.IsScoped && !service.Implementation.IsStatic) {
                builder.Append("((global::");
                builder.AppendNameSpace(serviceProvider.NameSpace);
        ...
        private readonly void AppendServicesGetterConstructor(Service service) {
            builder.Append(" => ");
            builder.Append('_');
            builder.AppendFirstLower(service.Name);
            builder.Append(';');
        ...
        private void AppendServicesGetterLazy(Service service, bool threadSafe) {
            builder.Append(" {\n");
        ...
        /// <param name="isScopeProvider"></param>
        /// <returns></returns>
        public readonly (bool hasDisposeList, bool hasAsyncDisposeList) AppendServicesTransient(DisposeGeneration generateDisposeMethods, bool isScopeProvider) {
            ...
            foreach (Service service in serviceProvider.TransientList) {
                ...
                switch ((service.IsDisposable, service.IsAsyncDisposable, generateDisposeMethods)) {
                    case (true, false, not DisposeGeneration.NoDisposing):
                        hasDisposeList = true;
                        AppendTransientGetterWithDisposeList("disposeList", service, isScopeProvider);
                        break;
                    case (_, true, not DisposeGeneration.NoDisposing):
                        hasAsyncDisposeList = true;
                        AppendTransientGetterWithDisposeList("asyncDisposeList", service, isScopeProvider);
                        break;
                    default:
                        builder.AppendServiceGetter(service);
                        builder.Append(" => ");
                        AppendServiceCreationTransient(service, isScopeProvider, indent.Sp4);
                        break;
                }
        ...
        private readonly void AppendTransientGetterWithDisposeList(string disposeListName, Service service, bool isScopeProvider) {
            if (service.GetAccessor == GetAccess.Property) {
                builder.Append(service.Name);
                builder.Append(" {\n");
                builder.Append(indent.Sp8);
                builder.Append("get {\n");
                AppendTransientGetterWithDisposeListCore(disposeListName, service, isScopeProvider, indent.Sp12);
                builder.Append(indent.Sp8);
                builder.Append('}');
                builder.Append('\n');
                builder.Append(indent.Sp4);
                ...
                builder.Append("()");
                builder.Append(" {\n");
                AppendTransientGetterWithDisposeListCore(disposeListName, service, isScopeProvider, indent.Sp8);
                builder.Append(indent.Sp4);
                builder.Append('}');
            }
        }

        private readonly void AppendTransientGetterWithDisposeListCore(string disposeListName, Service service, bool isScopeProvider, string indentation) {
            ...
            builder.AppendFirstLower(service.Name);
            builder.Append(" = ");
            AppendServiceCreationTransient(service, isScopeProvider, indentation);
            builder.Append('\n');

            if (!isScopeProvider ? serviceProvider.ThreadSafe : serviceProvider.ThreadSafeScope) {
                builder.Append(indentation);
                builder.Append("lock (");
        ...
        private readonly void AppendServiceCreationTransient(Service service, bool isScopeProvider, string indentation) {
        ...
        /// </summary>
        /// <param name="isScopeProvider"></param>
        public readonly void AppendServicesDelegate(bool isScopeProvider) {
            int currentPosition = builder.Length;
        ...
        /// </summary>
        /// <param name="singletonList"></param>
        /// <param name="generateDisposeMethods"></param>
        /// <param name="threadSafe"></param>
        ...
        public void AppendDisposeMethods(List<Service> singletonList, DisposeGeneration generateDisposeMethods, bool threadSafe, bool hasDisposeList, bool hasAsyncDisposeList, bool hasDisposeMethod, bool hasDisposeAsyncMethod) {
            ...
            if (generateDisposeMethods.HasFlag(DisposeGeneration.Dispose)) {
                ...
                if (hasDisposeList)
                    AppendDisposingDisposeList(threadSafe);

                if (hasAsyncDisposeList)
                    AppendDisposingAsyncDisposeListDiscard(threadSafe);
            ...
            if (generateDisposeMethods.HasFlag(DisposeGeneration.DisposeAsync)) {
                ...
                switch ((singeltonAsyncDisposablesCount, hasAsyncDisposeList)) {
                    ...
                    case (1, false): {
                        Service asyncDisposableService = singletonList.First((Service service) => service.IsAsyncDisposable);
                        if (singeltonDisposablesCount > 0) {
                            foreach (Service service in singletonList)
                    ...
                    case (0, true): {
                        ...
                        AppendDisposingAsyncDisposeListArray(threadSafe);
        ...
        private void AppendDisposingDisposeList(bool threadSafe) {
            if (threadSafe) {
                builder.Append(indent.Sp8);
        ...
        private void AppendDisposingAsyncDisposeListDiscard(bool threadSafe) {
            if (threadSafe) {
                builder.Append(indent.Sp8);
        ...
        private void AppendDisposingAsyncDisposeListArray(bool threadSafe) {
            if (threadSafe) {
                builder.Append(indent.Sp8);
                builder.Append("lock (asyncDisposeList)\n");
                indent.IncreaseLevel();
        ...
        public readonly void AppendUnsafeAccessorMethods(List<Service> serviceList) {
            int currentPosition = builder.Length;
        ...
        /// <param name="generateDisposeMethods"></param>
        /// <param name="threadSafe"></param>
        public readonly void AppendClassSummary(DisposeGeneration generateDisposeMethods, bool threadSafe) {
