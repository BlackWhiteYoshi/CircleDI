...
using ServiceProviderWithStartIndex = (CircleDI.Generation.ServiceProvider serviceProvider, int startIndex);
...
file static class RegisterServiceProviderAttributeExtension {
    public static void RegisterServiceProviderAttribute(this IncrementalGeneratorInitializationContext context, string serviceProviderAttributeName, IncrementalValueProvider<ImmutableArray<INamedTypeSymbol?>> componentList, ObjectPool<StringBuilder> stringBuilderPool) {
        ...
        // add components
        IncrementalValuesProvider<ServiceProviderWithStartIndex> serviceProviderWithComponentsList = serviceProviderWithExtraList
            .Combine(componentList).Select(ServiceProviderWithComponents);

        // init dependency tree
        IncrementalValuesProvider<ServiceProvider> serviceProviderList = serviceProviderWithComponentsList
            .Select((ServiceProviderWithStartIndex value, CancellationToken _) => value.serviceProvider.CreateDependencyTree()).WithComparer(NoComparison<ServiceProvider>.Instance);


        // generate attributes for automatic added razor components
        context.RegisterSourceOutput(serviceProviderWithComponentsList, (SourceProductionContext context, ServiceProviderWithStartIndex value) => context.GenerateComponentAttributes(stringBuilderPool, value.serviceProvider, value.startIndex));
    ...
    /// <param name="_">CancellationToken is not used</param>
    /// <returns></returns>
    private static ServiceProviderWithStartIndex ServiceProviderWithComponents((ServiceProviderWithExtra serviceProviderWithExtra, ImmutableArray<INamedTypeSymbol?> componentList) pair, CancellationToken _) {
        ...
        int startIndex = serviceProvider.TransientList.Count;
        ...
        return (serviceProvider, startIndex);
    }
    ...
    public static void GenerateComponentAttributes(this SourceProductionContext context, ObjectPool<StringBuilder> stringBuilderPool, ServiceProvider serviceProvider, int startIndex) {
        if (startIndex == -1)
            return;
        ...
        // components
        for (int i = startIndex; i < serviceProvider.TransientList.Count; i++) {
            Service service = serviceProvider.TransientList[i];

            builder.Append(indent.Sp0);
            builder.Append("[global::CircleDIAttributes.Transient<");
            builder.AppendOpenFullyQualified(service.ServiceType);
            builder.Append(">]\n");
        }
        ...
        string source = builder.ToString();
        string hintName = builder.CreateHintName(serviceProvider.Identifier, ".ComponentAttributes.g.cs");
        context.AddSource(hintName, source);
